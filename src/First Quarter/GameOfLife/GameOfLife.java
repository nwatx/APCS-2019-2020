...
public class GameOfLife extends JFrame implements  ActionListener
{
    ...
    public void initialize( )
    {
        ...
        newGameMenuItem    = new JMenuItem("Game");
    ...
    public GameOfLife( )
    {
        ...
        setTitle("Game of Life Neo Wang");
    ...
    public void actionPerformed(ActionEvent e)
    {
        ...
        {
            // set the generation to last
            generation = MAX_GENERATIONS-1;
            canEdit = false;  // let them edit the 0 generation
            displayGeneration(generation);
        }
    ...
    public int countNeighbors(int g, int r, int c)
    {
        ...
        // if you use nested loops, then remove the code below
                ...
                if (isValid(r-1,c) &&  board[g][r-1][c] == '*')
                {
                    numNeighbors++;
                }
                if (isValid(r-1,c+1) &&  board[g][r-1][c+1] == '*')
                ...
                if (isValid(r,c-1) &&  board[g][r][c-1] == '*')
                {
                    numNeighbors++;
                }
                if (isValid(r,c+1) &&  board[g][r][c+1] == '*')
    ...
    public boolean isValid(int r, int c)
    {
        ...
        if ( r < board[0].length && r >= 0 && c < board[0][0].length && c >= 0)
        return true;
        return false;
    ...
    public void displayGeneration(int whichGeneration) {
        ...
        // board[whichGeneration][r] would be the number of columns for generation whichGeneration on row r
            ...
            for (int r = 0; r < board[whichGeneration].length; r++) {
                for (int c = 0; c < board[whichGeneration][r].length; c++) {
    ...
    public void clearBoard()
    {
        ...
        {
        for ( int r = 0; r < board[generation].length; r++ )
        {
            for ( int c = 0; c < board[generation][r].length; c++ )
            {
                board[g][r][c] = ' ';
            }
        }
